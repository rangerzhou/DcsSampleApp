...
public abstract class SDKBaseActivity extends AppCompatActivity implements
    ...
    private DeviceControlDeviceModule.IDeviceControlListener deviceControlListener = new DeviceControlDeviceModule.IDeviceControlListener() {
        ...
        public void onSetBluetooth(SetBluetoothPayload payload) {
            Log.d(TAG, "Bluetooth status: " + payload.getBluetooth());
            BluetoothAdapter bluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
            if (payload.getBluetooth()) {
    ...
    private void launchAppForPackageName(String packname, TryLaunchAppPayload payload) {
    ...
    private AppDeviceModule.IAppListener appListener = new AppDeviceModule.IAppListener() {
        ...
        public void onTryLaunchApp(TryLaunchAppPayload payload) {
            Log.d(TAG, "AppDeviceModule-onTryLaunchApp--onLaunchApp AppName: " + payload.getAppName() + ", PackageName: " + payload.getPackageName());
            if ("微信".equals(payload.getAppName())) {
                launchAppForPackageName("com.tencent.mm", payload);
            } else if ("支付宝".equals(payload.getAppName())) {
                launchAppForPackageName("com.eg.android.AlipayGphone", payload);
            } else if ("王者荣耀".equals(payload.getAppName())) {
                launchAppForPackageName("com.tencent.tmgp.sgame", payload);
            } else {
                textViewRenderVoiceInputText.setText("没有安装" + payload.getAppName());
            }
        ...
        public void onLaunchApp(LaunchAppPayload payload) {
            Log.d(TAG, "AppDeviceModule-onLaunchApp--onLaunchApp AppName: " + payload.getAppName() + ", PackageName: " + payload.getPackageName());
        }
    };
...
//        getInternalApi().setOnPlayingWakeUpHighSensitivity(WAKEUP_ON_PLAYING_HIGH_SENSITIVITY);
        ...
        IAppLauncher appLauncher = AppLauncherImpl.getInstance(getApplicationContext());
        AppDeviceModule appDeviceModule = new AppDeviceModule(messageSender, appLauncher);
